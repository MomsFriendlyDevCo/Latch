...
export default class LatchSet {
	...
	* @param {AxiosRequest|Object} request The incomming request
	* @param {Object} [options] Options to mutate behaviour
	* @param {Function} [options.context] Function to extract the document being referred to, if any. Usually `req.params.id` but can be repurposed
	* @param {String} [options.maskKey='verb'] Key to set to the eventual extracted verb, prior to masking
	* @returns {LatchSet} A NEW LatchSet instance with the extracted mask applied
	*/
	maskFromRequest(request, options) {
		let settings = {
		...
		let req = { // Cleaned up version of request
			params: {},
			...request,
			method: request.method ? request.method.toUpperCase() : 'GET',
		};

		let contextID = settings.context.call(this, req); // Extract contextID
		let verb = // Try to figure out verb
			req.method == 'GET' && contextID == 'count' ? 'count'
			: req.method == 'GET' && contextID == 'meta' ? 'meta'
			: req.method == 'GET' && contextID ? 'get'
			: req.method == 'GET' && !contextID ? 'query'
			: req.method == 'POST' && contextID ? 'save'
			: req.method == 'POST' && !contextID ? 'create'
			: req.method == 'DELETE' && contextID ? 'delete'
			: false;

		return this.mask({
			[settings.maskKey]: verb,
		})
	}
	// }}}
